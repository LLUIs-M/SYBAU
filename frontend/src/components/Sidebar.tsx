...
export default function Sidebar({
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-50 lg:relative lg:z-auto lg:inset-auto ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"} ${isExpanded ? "w-72 lg:w-60" : "w-72 lg:w-14"} h-full bg-zinc-50 dark:bg-zinc-950 border-r border-zinc-200 dark:border-white/6 flex flex-col shrink-0 transition-all duration-200 overflow-hidden`}
    ...
    >
      ...
      <div className="px-3 py-2.5 border-t border-zinc-200 dark:border-zinc-700 flex items-center justify-between">
        <span
          className={`whitespace-nowrap overflow-hidden transition-all duration-200 text-[10px] text-zinc-400 dark:text-zinc-600 ${isExpanded ? "max-w-xs opacity-100" : "max-w-0 opacity-0"}`}
        >
          Powered by Ollama
        </span>
