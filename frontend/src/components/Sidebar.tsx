    ...
    >
      ...
      >
        ...
        {isExpanded && (
          <button
            ...
            className="p-1 rounded-md transition-colors cursor-pointer text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white"
      ...
      <div className="px-2">
        ...
        >
          ...
          <span
            className={`whitespace-nowrap overflow-hidden transition-all duration-300 ease-out ${isExpanded ? "max-w-xs opacity-100" : "max-w-0 opacity-0"}`}
          >
            New Chat
          </span>
        </button>
      ...
      {(isExpanded || mobileOpen) && (
        <div className="flex-1 overflow-y-auto px-2 pb-2">
          ...
          {GROUP_ORDER.filter((g) => groups[g]?.length).map((group) => (
            <div key={group} className="mb-1 flex flex-col gap-2">
      ...
      <div className="px-3 py-2.5 border-t border-zinc-200 dark:border-zinc-700 flex items-center justify-between">
        <span
          className={`whitespace-nowrap overflow-hidden transition-all duration-300 ease-out text-[10px] text-zinc-400 dark:text-zinc-600 ${isExpanded ? "max-w-xs opacity-100" : "max-w-0 opacity-0"}`}
        >
          Powered by Ollama
        </span>
